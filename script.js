...
(function () {
  ...
  // API endpoint for Google Apps Script data backend
  window.API_URL =
    window.API_URL ||
    "https://script.google.com/macros/s/AKfycbxZome1J9ewNlLZBBXabx6XqauM7aAGUxnZN6gdkvFCdUSE-UnSfeepRMjvIPFuKkS9/exec";
  const API_URL = window.API_URL;

  // helper: wrap fetch to notify user when the server is unreachable
  ...
  async function postToBackend(payload) {
    try {
      ...
      if (!response.ok) {
        const body = await response.text();
        throw new Error(
          `POST request failed: ${response.status} ${response.statusText} - ${body}`,
        );
      }

      return response.json();
    } catch (error) {
      ErrorHandler.handle(error, "postToBackend");
      throw error;
    }
  }

  async function postToBackendNoCors(payload) {
    try {
      const response = await fetch(API_URL, {
        method: "POST",
        mode: "no-cors",
      ...
      ErrorHandler.handle(error, "postToBackendNoCors");
      throw error;
    }
  }
  ...
  async function fetchMenu(cacheBuster = Date.now()) {
    try {
      const url = `${API_URL}?action=getMenu${cacheBuster ? `&t=${cacheBuster}` : ""}`;
      const response = await fetch(url);
      if (!response.ok) {
        const body = await response.text();
        throw new Error(
          `Failed to fetch menu: ${response.status} ${response.statusText} - ${body}`,
        );
      }
      ...
      menuItems = rawItems
        ...
        .map((item) => ({
          ...item,
          cat: item.cat || mapCategoryToKey(item["Category"] || item.category),
        }));
  ...
  async function addNewProduct(formData) {
    try {
      ...
      const rawCategory =
        ...
        values.type ||
        "food";
      const catLabel = mapCategoryToArabic(rawCategory);
      const catKey = mapCategoryToKey(rawCategory);
      const price =
        Number(
      ...
      const id = String(getNextMenuItemId()).trim();

      if (!id || !name) {
        throw new Error("Product ID and Item Name are required");
      }

      const payload = {
        action: "addItem",
        targetSheet: "Menu",
        rowData: [id, name, catLabel, price, img, "Active"],
      };

      await postToBackendNoCors(payload);
  ...
  async function submitOrder(orderDetails) {
    try {
      ...
      const rowData = [
        ...
        details.customerPhone || details.phone || "",
        details.customerAddress || details.address || "",
        details.items ? JSON.stringify(details.items) : "",
        details.total || 0,
        details.status || "New",
      ...
      showNotification("✅ تم إرسال الطلب إلى الطلبات بنجاح", "success");
      return result;
    } catch (error) {
  ...
  async function loadMenuItems() {
    const raw = await fetchJson(`?action=getMenu&t=${Date.now()}`);
  ...
  async function loadAllOrders() {
    const raw = await fetchJson("?action=getOrders");
  ...
  async function loadReviews() {
    const raw = await fetchJson("?action=getReviews");
    if (Array.isArray(raw)) {
      reviews = raw;
  ...
  // البيانات ستُحمّل من API_URL في وقت التشغيل
  ...
  function addToCart(id) {
    ...
    if (existingItem) {
      ...
      const cartItem = {
        ...
        cat: item.category || item.cat || "food",
        img:
          item.img ||
          item.image ||
  ...
  function renderCartItems() {
            ...
            <div class="cart-item" style="background:white; border:1px solid #E8EAED; border-radius:12px; padding:20px; margin-bottom:15px; display:flex; gap:15px; align-items:center; transition:all 0.3s;">
                ...
                <div style="flex-grow:1;">
                    ...
                    <div style="display:flex; align-items:center; gap:8px; margin-top:10px;">
                        <button data-action="decrease-qty" data-index="${index}" role="button" tabindex="0" aria-label="نقص الكمية" style="min-width:62px; height:36px; background:#F0F0F0; border:1px solid #DDD; border-radius:10px; cursor:pointer; font-weight:700; transition:0.2s;">− أقل</button>
                        <span style="width:44px; text-align:center; font-weight:700; color:#FF6B35;">${quantity}</span>
                        <button data-action="increase-qty" data-index="${index}" role="button" tabindex="0" aria-label="زيادة الكمية" style="min-width:62px; height:36px; background:#FF6B35; border:none; border-radius:10px; cursor:pointer; color:white; font-weight:700; transition:0.2s;">+ زيادة</button>
                    </div>
                </div>
    ...
    if (shippingPriceEl) shippingPriceEl.textContent = `${shipping} ج.م`;
  }
  ...
  function updateCartUI() {
    ...
    if (shippingPriceEl) {
      shippingPriceEl.textContent = `${shipping} ج.م`;
    }
    if (totalElement) {
  ...
  function finishOrder() {
    ...
    // إرسال الطلب إلى Google Apps Script عبر API_URL
    try {
      submitOrderToApi(orderData)
        .then((resp) => {
          console.log("✅ الطلب محفوظ في الخادم, response:", resp);
    ...
    if (typeof submitOrderToGoogleSheets === "function") {
      submitOrderToGoogleSheets(orderId, orderSummary, cart);
    }
        ...
        <div style="background:white; padding:30px; border-radius:12px; max-width:500px; width:90%; max-height:90vh; overflow-y:auto; box-shadow:0 10px 40px rgba(0,0,0,0.3); direction:rtl;">
            ...
            <div style="margin-bottom:16px;">
                ...
                <select id="editItemCategory" style="width:100%; padding:10px; border:2px solid #ECF0F1; border-radius:6px; font-size:14px; font-family:inherit; box-sizing:border-box;">
                    <option value="food" ${mapCategoryToKey(item["Category"] || item.category) === "food" ? "selected" : ""}>🍔 أطعمة</option>
                    <option value="drinks" ${mapCategoryToKey(item["Category"] || item.category) === "drinks" ? "selected" : ""}>🥤 مشروبات</option>
                    <option value="sweets" ${mapCategoryToKey(item["Category"] || item.category) === "sweets" ? "selected" : ""}>🍰 حلويات</option>
                </select>
            </div>
  ...
  async function saveEdit(id, editModal) {
    ...
    const newCatRaw = document.getElementById("editItemCategory")?.value;
  ...
  function editItemPrice(id, currentPrice) {
    ...
    if (item) {
      ...
      item.price = numPrice;
      console.log("✅ تم تحديث السعر في الذاكرة");
...
function enqueueSubmission(payload) {
  if (payload && payload.orderId && outboxHasOrder(payload.orderId)) {
    console.log(
      "🔁 تم تخطي إضافة طلب مكرر إلى طابور Google Sheets:",
      payload.orderId,
    );
    return null;
  }
...
function sendToGoogleSheets(payload) {
  const url =
    "https://script.google.com/macros/s/AKfycbxO0mSLXl-rlxovEoO1d-tz1TcwNwmtk7n9KaOGkmRPVTFOi_bSLc7zstEo4HsR0al0/exec";

  const jsonBody = JSON.stringify({
    orderId: payload.orderId || "ORD-" + Date.now(),
    customerName: payload.customerName || "بدون اسم",
    phone: payload.phone || "201021279663",
    address: payload.address || "بدون عنوان",
    email: payload.email || "",
